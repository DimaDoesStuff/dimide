/*---------------------------------------------------------------------------------------------
 *  Copyright (c) DimIDE Team. All rights reserved.
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to DimIDE"));
	...
	private _renderStep(): void {
		...
		switch (stepId) {
			// DimIDE: Removed AI steps (SignIn, AiPreference, AgentSessions)
	...
	private _updateButtonStates(): void {
		...
		if (this.nextButton) {
			if (this.currentStepIndex === 0) {
				// DimIDE: First step is Personalize, skip sign-in
		...
		if (this.footerLeft) {
			if (this._isLastStep()) {
				// DimIDE: No sign-in nudge (AI features removed)
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to DimIDE");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "A modern, PyCharm-inspired IDE.");
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all DimIDE commands.")
		);
	}
